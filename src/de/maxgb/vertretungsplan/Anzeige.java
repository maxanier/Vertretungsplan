...
public class Anzeige extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (username != "" && password != "" && klasse != "") {
			...
			if (isOnline()) {
				...
				webview = (WebView) findViewById(R.id.webView1); // Webview
																	// finden
																	// und
																	// speichern
			...
			setContentView(R.layout.activity_anzeige); // Layout laden
			webview = (WebView) findViewById(R.id.webView1); // Webview finden
																// und speichern
			webview.loadData(no_username, "text/html; charset=UTF-8", null); // Anzeige
																				// der
																				// Fehlermeldung,
																				// dass
																				// no_username
	...
	public boolean abrufen(HttpClient httpclient) throws Exception {
		try {
			...
			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) { // Erfolg
				...
				save(responseString, "debug_login.html"); // Speichern der Seite
															// zum Debuggen
	...
	public ArrayList<Vertretung> auswerten(File file) throws Exception {
		try {
			...
			for (int j = 0; j < font.getLength(); j += 3) { // Durchlaufen der
															// Font-Nodes, wobei
															// jede dritte Node
															// ein Tag ist
				String tag = font.item(j).getChildNodes().item(1)
						.getChildNodes().item(0).getNodeValue();// Auslesen des
																// Tags
														...
														// tr-Elemente
				{
					...
					// Attribute hat und dann ÃœberprÃ¼fen der class, ob es sich
	...
	public String anzeigen(ArrayList<Vertretung> vertretungen, String username,
		...
		if (vertretungen != null && vertretungen.size() > 0) {
			...
			String ergebnis = "<html><head><meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\" /></head><body><div align=\"center\"><i>"
					+ stand
					+ "</i><br>Datum: "
					+ tag
			...
			for (int i = 0; i < vertretungen.size(); i++) {
				...
				if (klasse.trim().equals("ALL")
					...
					ergebnis += "<th><font size=\"-1\">" + v.art
							+ "</font></th>  ";
					ergebnis += "<th><font size=\"-1\">" + v.fach
							+ "</font></th>  ";
					ergebnis += "<th><font size=\"-1\">" + v.raum
							+ "</font></th>  ";
					ergebnis += ("</tr>\n");
					gefunden = true;
			...
			if (!gefunden) {
				ergebnis = "<html><body><p style=\"padding-top:40%;\"><div align=\"center\">Keine Vertretungen fï¿½r die gewï¿½hlte Stufe/Klasse("
						+ klasse + ")</div></p></body></html>";
			}
			...
			throw new Exception(
					"Fehler: Vermutlich falscher Benutzername oder falsches Passwort gew&aum;lhlt:\n Nutzername: "
							+ username + " Passwort: *****");
		}
